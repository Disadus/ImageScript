...
(async () => {
    {
        ...
        if (process.env.OVERWRITE_TEST)
            await fs.writeFile('./src/tests/targets/fill-static.png', encoded);

        const target = await fs.readFile('./src/tests/targets/fill-static.png');
...

    {
        ...
        if (process.env.OVERWRITE_TEST)
            await fs.writeFile('./src/tests/targets/fill-func.png', encoded);

        const target = await fs.readFile('./src/tests/targets/fill-func.png');
