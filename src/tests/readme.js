...
(async () => {
    const [backgroundSVG, avatarBinary, badges, font] = await Promise.all([
        fs.readFile('./src/tests/svgs/background.svg').then(b => b.toString()),
        fs.readFile('./src/tests/targets/external.png'),
        Promise.all(
            [
                'crown', 'potato', 'mask', 'microbe', 'petri_dish', 'thermometer', 'cigarette'
            ].map(
                x => fs.readFile(`./src/tests/svgs/${x}.svg`)
                    .then(b => b.toString()))
        ),
        fs.readFile('./src/tests/fonts/carbon phyber.ttf')
    ]);
    ...
    if (process.env.OVERWRITE_TEST)
        await fs.writeFile('./src/tests/targets/readme.png', encoded);

    if (!(await fs.readFile('./src/tests/targets/readme.png')).equals(Buffer.from(encoded)))
        process.exit(1);
})();
