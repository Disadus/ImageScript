...
(async () => {
...

    {
        ...
        if (process.env.OVERWRITE_TEST)
            await fs.writeFile('./src/tests/targets/gif.gif', encoded);

        // const desired = await fs.readFile('./src/tests/targets/gif.gif');
        // if (!desired.equals(encoded))
        //     panic('encoding failed');
    }

    {
        const binary = await fs.readFile('./src/tests/targets/gif.gif');
