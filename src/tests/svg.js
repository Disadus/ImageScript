...
(async () => {
    const svg = await fs.readFile('./src/tests/svgs/potato.svg');
    ...
    if (process.env.OVERWRITE_TEST)
        await fs.writeFile('./src/tests/targets/potato.png', encoded);

    const target = await fs.readFile('./src/tests/targets/potato.png');
