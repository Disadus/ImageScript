...
(async () => {
    {
        ...
        if (process.env.OVERWRITE_TEST)
            await fs.writeFile('./src/tests/targets/rotate-45.png', encoded);

        await fs.writeFile('./src/tests/targets/rotate-45.png', encoded);
        const target = await fs.readFile('./src/tests/targets/rotate-45.png');
...

    {
        ...
        if (process.env.OVERWRITE_TEST)
            await fs.writeFile('./src/tests/targets/rotate-45-noresize.png', encoded);

        const target = await fs.readFile('./src/tests/targets/rotate-45-noresize.png');
...

    {
        ...
        if (process.env.OVERWRITE_TEST)
            await fs.writeFile('./src/tests/targets/rotate-180.png', encoded);

        await fs.writeFile('./src/tests/targets/rotate-180.png', encoded);
        const target = await fs.readFile('./src/tests/targets/rotate-180.png');
